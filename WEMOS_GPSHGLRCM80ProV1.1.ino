...
void loop()
{
  ...
  if (newData) 
  {
    ...
    getMapsLink(latitude, longitude);
    Serial.println("\n");
    inputStringGPS = "";
    newData = false;
  }
...
float getLatitude()
{
  ...
  float latitude = latitudeString.substring(0, 2).toFloat() + latitudeString.substring(2).toFloat() / 60.0;
...
float getLongitude()
{
  ...
  float longitude = longitudeString.substring(0, 3).toFloat() + longitudeString.substring(3).toFloat() / 60.0;
...
void getMapsLink(float latitude, float longitude)
{
  Serial.print("Maps Link: http://maps.google.com/maps?q=");
